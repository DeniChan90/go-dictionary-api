...
var userSettingsCollection *mongo.Collection = database.OpenCollection(database.Client, "user_settings")
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "BAD ERROR OCCURED :( "})
		}
...
func CreateUserSettings() gin.HandlerFunc {
	return func(c *gin.Context) {
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		...
		validationErr := validate.Struct(settings)
		...
		settings.User_id = c.Param("user_id")

		_, insertErr := userSettingsCollection.InsertOne(ctx, settings)

		if insertErr != nil {
			msg := fmt.Sprintf("User Settings were not created.")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
...
func GetUserSettings() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		var settings models.UserSettings
		err := userSettingsCollection.FindOne(ctx, bson.M{"user_id": userId}).Decode(&settings)
		defer cancel()
